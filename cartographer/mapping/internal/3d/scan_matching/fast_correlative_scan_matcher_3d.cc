...
FastCorrelativeScanMatcher3D::MatchFullSubmap(
  ...
  const SearchParameters search_parameters{
      linear_window_size, common::RoundToInt(2 * options_.linear_z_search_window() / resolution_), M_PI/5, &low_resolution_matcher};
  return MatchWithSearchParameters(
      search_parameters,
...
std::vector<DiscreteScan3D> FastCorrelativeScanMatcher3D::GenerateDiscreteScans(
  ...
  //}
  if (search_parameters.angular_search_window==M_PI && cand_set.size()>=10) {
    result.clear();
    int n = 0;
